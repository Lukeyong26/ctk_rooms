...
const RoomCard: React.FC<RoomCardProps> = ({ id, imageUrl, title, desc, bookings }) => {
  ...
  return (
    <div className='relative bg-[#E5E1DA] h-80 w-full dark:bg-gray-900 rounded-lg shadow-lg'>
      <div className='relative h-6/8 w-full'>
        <div className='absolute z-1 flex flex-col w-full h-full'>
          ...
          <div className='flex flex-col p-4 w-full h-full mt-auto text-white overflow-x-auto'>
      ...
      <div className='flex flex-row h-2/8 gap-2 p-4 w-full'>
        <Link to={'/rooms/'+id} className='h-full w-full'><button className="btn w-full h-full shadow-sm">See Room Bookings</button></Link>
        <Link to={'/bookings/'+id} className='h-full w-full'><button className="btn w-full h-full shadow-sm">Book Room</button></Link>
      </div>
    </div>
  );
};
